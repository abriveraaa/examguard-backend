...
public class ExamPortfolioPageEvent extends PdfPageEventHelper {
    ...
    public void onEndPage(PdfWriter writer, Document document) {
        if (writer.getPageNumber() == 1) {
            drawFirstPageHeader(writer);
            drawFirstPageFooter(writer);
        } else {
    ...
    private void drawFirstPageHeader(PdfWriter writer) {
        ...
        try {
            ...
            bagong.setAbsolutePosition(
                    page.getRight() - 35f - 82.8f,
                    top - 98f
            );
        ...
        canvas.moveTo(35f, 685);
        canvas.lineTo(page.getRight() - 35f, 685);
        canvas.stroke();
        canvas.restoreState();
    }

    private void drawFirstPageFooter(PdfWriter writer) {
        PdfContentByte canvas = writer.getDirectContent();
        Rectangle page = writer.getPageSize();

        float edgeMargin = 35f;
        ...
        try {
            ...
            socotec.setAbsolutePosition(
                    page.getRight() - 72f - 93f,
                    48f
            );
    ...
    private void drawPageNumber(PdfWriter writer) {
        ...
        try {
            ...
            BaseFont bold = BaseFont.createFont(
                    getClass()
                            .getResource("/fonts/NotoSans-Bold.ttf")
                            .toString(),
                    BaseFont.IDENTITY_H,
                    BaseFont.EMBEDDED
            );
